    ...
    @if (session('exitoRegistro'))
        <script>
            swal({
                ...
                text: "Se registro correctamente el evento",
                icon: "success",
                button: "OK",
            });
        </script>
                            ...
                            <div class="col-lg-7 col-xs-12 subjects-container">
                                ...
                                <div class="row">
                                    @if ($formativa->isNotEmpty())
                                        <p class="d-flex justify-content-center mt-0">
                                            {{ $formativa->first()->nombreArea }}</p>
                                        @foreach ($formativa as $area)
                                            <div class="col-lg-4 col-xs-12 ">
                                                <div class="form-check mt-3">
                                                    <input class="form-check-input" type="checkbox"
                                                        value="{{ $area->idAreas }}"
                                                        id="checkSubject{{ $area->idAreas }}" name="areas[]"
                                                        {{ in_array($area->idAreas, old('areas', [])) ? 'checked' : '' }}>
                                                    <label class="form-check-label"
                                                        for="checkSubject{{ $area->idAreas }}">
                                                        {{ $area->nombre }}
                                                    </label>
                                                </div>
                                            </div>
